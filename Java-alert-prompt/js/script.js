...
euro = 0.87;
document
  .getElementById("converterButton")
...
document.getElementById("flashButton").addEventListener("click", function () {
  ...
  alert(
    `${volume} файлов размером в 820 мб , поместиться на флэшке размером в ${userFlash} ГБ`
  );
});

document
  .getElementById("chocolateButton")
  .addEventListener("click", function () {
    ...
    alert(
      `Вы сможете купить ${chocolate} шоколадок. Остаток средств ${remains}`
    );
  });
